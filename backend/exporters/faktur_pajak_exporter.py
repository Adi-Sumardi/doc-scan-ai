...
class FakturPajakExporter(BaseExporter):
    ...
    def _populate_excel_sheet(self, ws, result: Dict[str, Any]):
        """Helper to populate worksheet with structured Faktur Pajak data - FLAT TABLE (NO MERGE)"""
        # Define styles
        ...
        # ===== FLAT TABLE: ALL DATA REPEATED PER ITEM ROW (NO MERGE) =====
        # If no items, create one row with N/A for item columns
        if not items or len(items) == 0:
            items = [{'description': 'N/A', 'quantity': '-', 'unit_price': '-'}]

        first_data_row = row

        # Write ONE ROW PER ITEM (all columns 1-19 filled, NO MERGE)
        for item_idx, item in enumerate(items):
            current_row = row + item_idx

            # Columns 1-15: Main faktur data (REPEATED for each item)
            main_data = [
            ...
            # Write main data (columns 1-15) - NO MERGE, repeated per row
